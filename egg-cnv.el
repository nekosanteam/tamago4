...
(egg-add-message
 '((nil
   ...
   (Chinese-GB
    ...
    (rcfile-error  "在读取 egg-startup-file 时，有出错发生了")
    (candidate     "候补:")
    (register-str  "汉字:")
    (register-yomi "辞典登录『%s』 读法:")
    (registered    "辞典项目『%s』(%s: %s)已被登录到 %s 中了"))
   (Chinese-CNS
    (no-rcfile     "在 %S 上沒有 egg-startup-file")
    (rcfile-error  "在讀取 egg-startup-file 時，有出錯發生了")
    (register-str  "漢字:")
    (candidate     "候補:")
    (register-yomi "辭典登錄『%s』  讀法:")
    (registered    "辭典項目『%s』(%s: %s)已被登錄到 %s 中了"))
   (Korean
    (no-rcfile     "%S 에 egg-startup-file 이 없읍니다")
    (rcfile-error  "egg-startup-file 에 에러가 발생했읍니다")
    (candidate     "후보:")
    (register-str  "한자:")
    (register-yomi "사전등록『%s』  독법:")
    (registered    "사전항목『%s』(%s: %s)을 %s에 등록했읍니다"))))
